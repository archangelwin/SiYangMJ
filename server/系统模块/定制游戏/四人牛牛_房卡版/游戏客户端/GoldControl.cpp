...
{
	LONGLONG lGold=0L;
	for (LONGLONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*(LONGLONG)pow((float)10L,(float)i);
	return lGold;
}
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=(LONGLONG)pow((float)10L,(float)iCellPos);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=9L*(LONGLONG)pow((float)10L,(float)iCellPos);
		if ((GetGold()+lAddGold)>m_lMaxGold) return;
	}
