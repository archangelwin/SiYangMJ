...
{
  if(m_TextureLeftCard.IsNull()==false)
  {
    m_TextureLeftCard.Destory();
  }
  if(m_TextureRightCard.IsNull()==false)
  {
    m_TextureRightCard.Destory();
  }
  return;
}
//获取大小
...
//  CardSize=(bSamllMode==false)?m_CardSize:m_CardSizeSmall;
  if(cbCardType == CARD_TYPE_NOMAL)
  {
    CardSize = m_CardSize;
  }
  if(cbCardType == CARD_TYPE_SMALL)
  {
    CardSize = m_CardSizeSmall;
  }
  if(cbCardType == CARD_TYPE_LEFT)
  {
    CardSize = m_CardSizeLeft;
  }
  if(cbCardType == CARD_TYPE_RIGHT)
  {
    CardSize = m_CardSizeRight;
  }
  if(cbCardType == CARD_TYPE_LOUT)
  {
    CardSize = m_CardSizeLeftOut;
  }
  if(cbCardType == CARD_TYPE_ROUT)
  {
    CardSize = m_CardSizeRightOut;
  }
  if(cbCardType == CARD_TYPE_OUT)
  {
    CardSize = m_CardSizeOut;
  }
...
VOID CCardResource::GetCardTexture(BYTE cbCardType, CD3DSprite ** pD3DTexture)
{
  ...
  if(cbCardType == CARD_TYPE_LEFT)
  {
    *pD3DTexture = &m_TextureLeftCard;
  };
  if(cbCardType == CARD_TYPE_RIGHT)
  {
    *pD3DTexture = &m_TextureRightCard;
  };
  if(cbCardType == CARD_TYPE_LOUT)
  {
    *pD3DTexture = &m_TextureLeftOutCard;
  };
  if(cbCardType == CARD_TYPE_ROUT)
  {
    *pD3DTexture = &m_TextureRightOutCard;
  };
  if(cbCardType == CARD_TYPE_OUT)
  {
    *pD3DTexture = &m_TextureOutCard;
  };
...
bool CCardControl::ShowOutCard(BYTE cbCardData[], BYTE cbCardCount)
{
  ...
  if((cbCardCount==0)||(cbCardCount>CountArray(m_CardItemArray)))
  {
    return false;
  }
  ...
  if((cbShootCount>0)&&(cbShootCount<m_cbCardCount))
  {
    ...
    for(BYTE i=0; i<m_cbCardCount; i++)
    {
      if(CardItemTemp[i].bShoot==false)
      {
        ...
        //调整索引
        if(cbNormalIndex==cbTargerItem)
        {
          cbNormalIndex=cbTargerItem+cbShootCount;
        }
      }
      else
      {
...
bool CCardControl::SwitchCardItem(BYTE cbSourceItem, BYTE cbTargerItem)
{
  ...
  if((cbSourceItem>=m_cbCardCount)||(cbTargerItem>=m_cbCardCount))
  {
    return false;
  }
...
BYTE CCardControl::GetShootCard(BYTE cbCardData[], BYTE cbBufferCount)
{
  ...
  for(BYTE i=0; i<m_cbCardCount; i++)
  {
    ...
    if(m_CardItemArray[i].bShoot==true)
    {
      cbCardData[cbShootCount++]=m_CardItemArray[i].cbCardData;
    }
  }
...
VOID CCardControl::CartoonMovie()
{
  if((m_bOutCard==true)&&(m_OutCardLapseCount.GetLapseCount(20)>0L))
  {
    ...
    if((m_cbCardCount==0)||(m_wOutCardIndex>=((m_cbCardCount-1)*DISPATCH_DELAY_INDEX+DISPATCH_MAX_INDEX)))
    {
      ...
      {
        pGameFrameEngine->SendMessage(IDM_OUT_CARD_FINISH,(WPARAM)this,0L);
      }
    }
  }
...
VOID CCardControl::GetOriginPoint(CPoint & OriginPoint)
{
  ...
  else
  {
    switch(m_XCollocateMode)
    {
      case enXLeft:
      {
        OriginPoint.x=m_BenchmarkPos.x+(MAX_COUNT-m_cbCardCount)*m_nXDistance/2;
        break;
      }
      case enXCenter:
      {
      ...
      case enXRight:
      {
        OriginPoint.x=m_BenchmarkPos.x-ControlSize.cx+(MAX_COUNT-m_cbCardCount)*m_nXDistance/2;
        break;
      }
    }
  }
...
BYTE CCardControl::SwitchCardPoint(CPoint & MousePoint)
{
  ...
  if((m_bHorizontal==true)&&(m_cbCardCount>0))
  {
    ...
    for(BYTE i=0; i<=cbCardIndex; i++)
    {
      ...
      //显示判断
      if(cbCurrentIndex>=m_cbShowCount)
      {
        continue;
      }

      //横向测试
      if(nXPos>(INT)(cbCurrentIndex*m_nXDistance+CardSize.cx))
      {
        break;
      }
      ...
      if((bShoot==true)&&(nYPos<=CardSize.cy))
      {
