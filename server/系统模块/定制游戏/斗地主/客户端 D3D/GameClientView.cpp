...
VOID CGameClientView::ResetGameView()
{
  ...
  if(m_TextureLastTurn.IsNull()==false)
  {
    m_TextureLastTurn.Destory();
  }
  if(m_TextureCallScore.IsNull()==false)
  {
    m_TextureCallScore.Destory();
  }
  if(m_TextureCartoonBomb.IsNull()==false)
  {
    m_TextureCartoonBomb.Destory();
  }
  if(m_TextureDispatchCard.IsNull()==false)
  {
    m_TextureDispatchCard.Destory();
  }
  if(m_TextureHistoryScore.IsNull()==false)
  {
    m_TextureHistoryScore.Destory();
  }
  if(m_TextureReversalCard.IsNull()==false)
  {
    m_TextureReversalCard.Destory();
  }
  if(m_TextureWaitCallScore.IsNull()==false)
  {
    m_TextureWaitCallScore.Destory();
  }
  if(m_TextureRocket.IsNull()==false)
  {
    m_TextureRocket.Destory();
  }
  if(m_TextureRocketLight.IsNull()==false)
  {
    m_TextureRocketLight.Destory();
  }
...
VOID CGameClientView::CartoonMovie()
{
  ...
  if((m_bCartoonPlan==true)&&(m_CartoonPlanLapseCount.GetLapseCount(10)>0L))
  {
    ...
    if(m_ptCartoonPlan.x < -200)
    {
      ...
      if(m_TextureCartoonPlan.IsNull()==false)
      {
        m_TextureCartoonPlan.Destory();
      }
    }
  }
  ...
  if((m_bCartoonRocket==true)&&(m_CartoonRocketLapseCount.GetLapseCount(30)>0L))
  {
    ...
    if(m_ptCartoonRocket.y < -550)
    {
      ...
      if(m_TextureCartoonRocket.IsNull()==false)
      {
        m_TextureCartoonRocket.Destory();
      }
      if(m_TextureRocket.IsNull()==false)
      {
        m_TextureRocket.Destory();
...
VOID CGameClientView::ShowBankerCard(BYTE cbCardData[3])
{
  ...
  if(m_TextureReversalCard.IsNull()==true)
  {
    m_TextureReversalCard.LoadImage(&m_D3DDevice,AfxGetInstanceHandle(),TEXT("REVERSAL_CARD"),TEXT("PNG"));
  }
...
VOID CGameClientView::SetLastTurnState(bool bLastTurnState)
{
  if(m_bLastTurnState!=bLastTurnState)
  {
    ...
    if(m_bLastTurnState==true)
    {
      ...
      if(m_TextureLastTurn.IsNull()==true)
      {
        m_TextureLastTurn.LoadImage(&m_D3DDevice,AfxGetInstanceHandle(),TEXT("LAST_TURN"),TEXT("PNG"));
      }
    }
    else
    {
...
VOID CGameClientView::SetWaitCallScore(bool bWaitCallScore)
{
  if(m_bWaitCallScore!=bWaitCallScore)
  {
    ...
    if(m_bWaitCallScore==true)
    {
      ...
      if(m_TextureWaitCallScore.IsNull()==true)
      {
        m_TextureWaitCallScore.LoadImage(&m_D3DDevice,AfxGetInstanceHandle(),TEXT("WAIT_CALL_SCORE"),TEXT("PNG"));
      }
    }
    else
    {
...
VOID CGameClientView::OnLButtonUp(UINT nFlags, CPoint Point)
{
  ...
  if(m_bMoveMouse==false)
  {
    ...
    if(wMouseDownItem==m_cbMouseDownItem)
    {
      pCardItem=m_HandCardControl[MYSELF_VIEW_ID].GetCardFromPoint(Point);
    }
...
VOID CGameClientView::OnMouseMove(UINT nFlags, CPoint MousePoint)
{
  ...
  if(m_cbHoverCardItem!=INVALID_ITEM)
  {
    ...
    //移动变量
    if(cbHoverCardItem!=m_cbHoverCardItem)
    {
      m_bMoveMouse=true;
    }
  }
    ...
    if(m_cbMouseDownItem!=INVALID_ITEM)
    {
      ...
      if(pCardItem->bShoot==true)
      {
        ...
        for(BYTE i=0; i<m_cbMouseDownItem; i++)
        {
          ...
          if((pCardItemTemp!=NULL)&&(pCardItemTemp->bShoot==true))
          {
            m_cbSwitchCardItem++;
          }
        }
...
VOID CGameClientView::SetUserAvatar(bool bCellScoreEnd)
{
  ...
  if(m_bCellScoreEnd == true)
  {
    ...
    if(m_TextureUserPlayer.IsNull() == false)
    {
      m_TextureUserPlayer.Destory();
    }
  ...
  {
    if(m_TextureUserPlayerGame.IsNull() == false)
    {
      m_TextureUserPlayerGame.Destory();
    }
...
VOID CGameClientView::ShowCartoonLine(bool bCartoon,BYTE cbCardType)
{
  ...
  else
  {
    ...
    if(m_bCartoonRocket==true)
    {
      ...
      //卸载资源
      ASSERT(m_TextureCartoonLine.IsNull()==false&& m_TextureLine.IsNull() == false);
      if(m_TextureCartoonLine.IsNull()==false)
      {
        m_TextureCartoonLine.Destory();
      }
      if(m_TextureLine.IsNull()==false)
      {
        m_TextureLine.Destory();
      }
