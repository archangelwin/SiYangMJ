...
#else
#define TIME_START_GAME       10                  //开始定时器
...
bool CGameClientEngine::OnSubChipResult(const void * pBuffer, WORD wDataSize)
{
  ...
  if(pChipResult->bTotal)
  {
    for(int i = 0; i < GAME_PLAYER; i++)
    {
      ...
      strInfo.Format(TEXT("玩家 %s %s"), m_szAccounts[i], (i==m_wBankerUser?TEXT("买底"):TEXT("顶底")));
      if(m_pIStringMessage!=NULL)
      {
...
bool CGameClientEngine::OnSubTrustee(const void * pBuffer,WORD wDataSize)
{
  ...
  if((IsLookonMode()||(pTrustee->wChairID!=GetMeChairID()))&&pUserItem!=NULL)
  {
    ...
    {
      szBuffer.Format(TEXT("玩家[%s]选择了托管功能."),pUserItem->GetNickName());
    }
    else
    {
      szBuffer.Format(TEXT("玩家[%s]取消了托管功能."),pUserItem->GetNickName());
    }
    if(m_pIStringMessage!=NULL)
    {
...
bool CGameClientEngine::VerdictOutCard(BYTE cbPreCardData,BYTE cbAlreadyCardData[7],BYTE cbZiCardCount)
{
  ...
  if(m_pIStringMessage!=NULL)
  {
    ...
    {
      m_pIStringMessage->InsertSystemString(TEXT("必须先出完箭牌！"));
    }
    else if(bRet == 4)
    {
      m_pIStringMessage->InsertSystemString(TEXT("必须先出完手牌中的风牌和箭牌！"));
    }
  }
