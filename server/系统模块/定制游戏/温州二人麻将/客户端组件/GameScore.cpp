...
void CGameScore::OnPaint()
{
  ...
  if(m_ScoreInfo.wProvideUser != INVALID_CHAIR)
  {
    CRect rcDraw;
    CString strTotal;
    strTotal.Format(TEXT("玩家：%s 胡了！"), m_ScoreInfo.szUserName[m_ScoreInfo.wChiHuUser]);
    ...
    {
      DCBuffer.DrawText(TEXT("软胡"),&rcChiHuRight,DT_SINGLELINE|DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER);
    }
    ...
    LPCTSTR pszKind[] =
    {
      TEXT("碰碰胡"),TEXT("八对"),TEXT("地胡"),TEXT("天胡"),TEXT("单吊"),
      TEXT("三金倒"),TEXT("双翻"),TEXT("清一色"),TEXT("硬八对")
    };
    //牌型权位
