...
class CTableFrameSink : public ITableFrameSink, public ITableUserAction
{
	//模式变量
protected:
	BYTE							m_cbMaxZui;							// 最大嘴数
	BYTE							m_cbScoreZui;						// 嘴对应的分数
	BYTE							m_cbPlayerCount;					//指定游戏人数，2-4
	CMD_S_RECORD					m_stRecord;
	BYTE							m_cbChooseZuiInfo[GAME_PLAYER][MAX_ZUI];        // 每个玩家选嘴情况
	BYTE							m_cbChooseZuiStatus[GAME_PLAYER];			   // 每个玩家选嘴情况
	//游戏变量
protected:
	WORD							m_wSiceCount;									//骰子点数
	WORD							m_wBankerUser;									//庄家用户
	WORD							m_wLastBankerUser;								//上次庄家用户
	WORD							m_wLastBankerUserTemp;								//上次庄家用户
...
protected:
	...
	BYTE							m_cbWinCount;
	WORD							m_wWinUser[GAME_PLAYER];							// 胡牌用户
...
protected:
	...
	bool							m_bNotFirstRepairFlowerRound[GAME_PLAYER];				//是否第一轮补花
	...
	BYTE							m_cbHandFlowerCard[GAME_PLAYER][MAX_FLOWER_CNT];		//花朵个数
	BYTE							m_cbFlowerCount[GAME_PLAYER];							//花朵个数
	BYTE							m_cbTatalFlowerCount[GAME_PLAYER];						//花朵总个数
...
public:
	...
	tagGangScore					m_GangScore[GAME_PLAYER];				//杠分
	SCORE							m_lDetailScore[GAME_PLAYER][GAME_PLAYER];		//每个杠得分
	SCORE							m_lUserGangScore[GAME_PLAYER];					//游戏中杠的输赢
	SCORE							m_lZuiScore[GAME_PLAYER][MAX_ZUI];				// 每个玩家选嘴情况
	//结束信息
protected:
...
public:
	...
	//补花
	BYTE IsShowFlower(WORD chairId);

	WORD GetSanBaoUser(WORD winUser, const CChiHuRight &chr);

	//摸牌
	BYTE GetBackCard() { return m_cbRepertoryCard[--m_cbLeftCardCount]; }

	//游戏结束
	virtual bool OnEventGameConclude(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbReason);

	void logConclude(const CMD_S_GameConclude &GameConclude);

	//发送场景
	virtual bool OnEventSendGameScene(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbGameStatus, bool bSendSecret);

	virtual void PersonalTableEnd();
	//事件接口
public:
	//时间事件
...
protected:
	// 用户选牌
	bool OnUserChooseZui(WORD wChairID, BYTE cbChooseZuiInfo[]);
	//用户出牌
	...
	void logCardData(WORD wTargetUser);

	void FlowerCntLogic(WORD chairId);
	// 
// 	bool listenLogic(WORD wChairID, BYTE cbOperateCard, BYTE cbOperateCode);
// 
// 	void checkTingAction(BYTE cbCharid, BYTE cbBaoTingCard);
// 
// 	void updateTingInfo(BYTE charId);
// 
// 	bool SearchTingCard(tagTingCardResult &TingCardResult, tagTingCardResult &AllTingCardResult, BYTE cbCharId);
...
protected:
	//发送操作
	bool SendOperateNotify(WORD chairId = INVALID_CHAIR);
	//取得扑克
	BYTE GetSendCard(bool bTail = false);
	//派发扑克
	bool DispatchCardData(WORD wSendCardUser,bool bTail = false);

	WORD getGuoZhuangId(WORD wChairId);

	void EstimateUserOpertion(WORD wCurrentUser, bool isSendCard);

	void userGangEstimate(WORD wCurrentUser, bool send);
	...
	void CalZuiScore(LONGLONG lEndScore[GAME_PLAYER]);

	string getZuiStr(enZuiKind zuiKind);
	//权位过滤
