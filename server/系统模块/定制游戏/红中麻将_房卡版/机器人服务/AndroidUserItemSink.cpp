...
//游戏时间
#define IDI_OUT_CARD				(100+0)			//出牌时间
#define IDI_START_GAME				(100+1)			//开始时间
#define IDI_OPERATE_CARD			(100+2)			//操作时加
	...
	case GAME_SCENE_PLAY:	//游戏状态
		{
			...
			//扑克变量
			CopyMemory(m_cbWeaveCount,pStatusPlay->cbWeaveItemCount,sizeof(m_cbWeaveCount));
			CopyMemory(m_WeaveItemArray,pStatusPlay->WeaveItemArray,sizeof(m_WeaveItemArray));
			m_GameLogic.SwitchToCardIndex(pStatusPlay->cbCardData,pStatusPlay->cbCardCount[wMeChairID],m_cbCardIndex[m_pIAndroidUserItem->GetChairID()]);
...
bool CAndroidUserItemSink::OnSubGameStart(void * pData, WORD wDataSize)
{
	...
	m_GameLogic.SwitchToCardIndex(&pGameStart->cbCardData[wMeChairId],cbCardCount,m_cbCardIndex[wMeChairId]);
...
bool CAndroidUserItemSink::OnSubOperateResult( const void *pBuffer,WORD wDataSize )
{
	...
	if ((pOperateResult->cbOperateCode&WIK_GANG)!=0)
	{
		...
		for (BYTE i=0;i<m_cbWeaveCount[wOperateUser];i++)
		{
			...
			{
				cbWeaveIndex=i;
				m_WeaveItemArray[wOperateUser][cbWeaveIndex].cbParam =TRUE;
		...
		if (cbWeaveIndex==0xFF)
		{
			...
			m_WeaveItemArray[wOperateUser][cbWeaveIndex].cbParam =cbPublicCard;
	...
	if (m_wCurrentUser==m_pIAndroidUserItem->GetChairID())
	{
		...
		{
			//BYTE cbOperateCard1[3] = {0,0,0};

			////发送命令
			//CMD_C_OperateCard OperateCard;
			//OperateCard.cbOperateCode=cbActionMask;
			//CopyMemory( OperateCard.cbOperateCard,cbOperateCard1,sizeof(cbOperateCard1) );
...
void CAndroidUserItemSink::OnOperateCard( BYTE cbOperateCode,BYTE cbOperateCard )
{
	...
	CopyMemory( &cbOperateCard1[0],&cbOperateCard,sizeof(BYTE) );
	CopyMemory( &cbOperateCard1[1],(((cbOperateCard==0)&&(cbOperateCode&WIK_GANG)))?&GangCardResult.cbCardData[0]:&cbOperateCard,sizeof(BYTE)*2 );
	CopyMemory( OperateCard.cbOperateCard,cbOperateCard1,sizeof(cbOperateCard1) );
