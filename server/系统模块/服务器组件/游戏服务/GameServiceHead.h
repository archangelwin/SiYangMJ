...
public:
	...
	//坐下动作
	virtual bool PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, DWORD dwSocketID = 0, LPCTSTR lpszPassword=NULL)=NULL;
...
public:
	...
	virtual void PersonalTableEnd() = NULL;
	//事件接口
public:
	//时间事件
...
public:
	...
	//取消创建
	virtual VOID CancelCreateTable(DWORD dwUserID[], BYTE cbPlayerCnt, BYTE cbTableType, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID,  TCHAR * szRoomID)=NULL;
	//开始游戏写入参与信息
...
struct tagTableFrameParameter
{
	...
	IDBCorrespondManager *          pIDBCorrespondManager;            //数据协调
