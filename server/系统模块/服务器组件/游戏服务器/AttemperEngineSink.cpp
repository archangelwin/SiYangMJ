...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GR_Join_TableAA_SUCCESS:			//创建成功
		{
			return OnDBJoinTableAASucess(dwContextID, pData, wDataSize);
		}
	case DBO_GR_Join_TableAA_FAILURE:			//创建失败
		{
			return OnDBJoinTableAAFailure(dwContextID, pData, wDataSize);
		}
...
//取消答复
bool CAttemperEngineSink::OnTCPNetworkSubQuitRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_QuiteRoom));
	if (wDataSize != sizeof(CMD_GR_QuiteRoom)) return false;
		...
		if (pIServerUserItem != NULL)
		{
			...
			if (wTableID != INVALID_TABLE || (bImmediteMatch == true && cbMatchStatus == MUS_PLAYING) || (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL && wDrawCount != 0))
			{
				...
				if (wTableID < m_pGameServiceOption->wTableCount)
				{
					...
					m_TableFrameArray[wTableID]->OnEventUserOffLine(pIServerUserItem, true);
				}
				else //先不处理看有什么问题
				{
...
bool CAttemperEngineSink::OnEventUserItemStatus(IServerUserItem * pIServerUserItem, WORD wOldTableID, WORD wOldChairID)
{
	...
	{
		if (UserStatus.UserStatus.wTableID == INVALID_TABLE)
		{
			UserStatus.UserStatus.wTableID = wOldTableID;
		}
		if (UserStatus.UserStatus.wChairID == INVALID_CHAIR)
		{
			UserStatus.UserStatus.wChairID = wOldChairID;
		}
	}
...
bool CAttemperEngineSink::OnDBCreateSucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	pTableFrame->SetTableType(pCreateSuccess->cbTableType);
	...
	{
		//执行坐下
		if(pTableFrame->PerformSitDownAction(wChairID, pIServerUserItem, dwContextID, pCreateSuccess->szPassword)== FALSE)
		{
			...
			if (pTableFrame->GetTableType() == 0)
			{
				//退还费用
				DBR_GR_CancelCreateTable CancelCreateTable;
				ZeroMemory(&CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
				CancelCreateTable.dwUserID[0] = pCreateSuccess->dwUserID;
				CancelCreateTable.cbTableType = 0;
...
bool CAttemperEngineSink::OnDBCreateFailure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	CTableFrame* pTableFrame = m_TableFrameArray[pCreateFailure->dwTableID];
	if (pTableFrame)
	{
		pTableFrame->SetPersonalTableLlocked(false);
		pTableFrame->SetPersonalTable(0, 0, 0);
...
//创建成功
bool CAttemperEngineSink::OnDBJoinTableAASucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GR_Join_TableAA_Success));
	if (wDataSize != sizeof(DBO_GR_Join_TableAA_Success)) return false;
...
// 	if ((pBindParameter->dwSocketID != dwContextID) || (pBindParameter->pIServerUserItem == NULL)) return true;
	...
	//获取用户
	if (GetBindUserItem(LOWORD(dwContextID)) == NULL)
	{
		ASSERT(GetBindUserItem(LOWORD(dwContextID)) != NULL);
	}
	IServerUserItem * pIServerUserItem = GetBindUserItem(LOWORD(dwContextID));
	...
	DBO_GR_Join_TableAA_Success* pCreateSuccess = (DBO_GR_Join_TableAA_Success*)pData;

	//获取信息
	CTableFrame* pTableFrame = m_TableFrameArray[pCreateSuccess->dwTableID];
	pTableFrame->PerfomSitDown(pIServerUserItem, pCreateSuccess->dwChairId);
	return true;
}

//创建失败
bool CAttemperEngineSink::OnDBJoinTableAAFailure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GR_CreateFailure));
	if (wDataSize != sizeof(DBO_GR_CreateFailure)) return false;
...
bool CAttemperEngineSink::OnDBCurrenceRoomCardAndBeant(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	CTableFrame* pTableFrame = m_TableFrameArray[pCardAndBeans->dwTableID];
	ASSERT(pTableFrame != NULL);
	...
	DWORD userId = pCardAndBeans->dwUserID;
	IServerUserItem* pUserItem = pTableFrame->SearchUserItem(userId);
	...
	m_pITCPNetworkEngine->SendData(pBind->dwSocketID, MDM_GR_PERSONAL_TABLE, SUB_GR_CURRECE_ROOMCARD_AND_BEAN, &CurrenceRoomCardAndBeans, sizeof(CMD_GR_CurrenceRoomCardAndBeans));
...
bool CAttemperEngineSink::OnTCPNetworkMainPersonalTable(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_QUIT_TABLE_ROOM:
		{
			return OnTCPNetworkSubQuitRoom(pData, wDataSize, dwSocketID);
		}
...
// 	}
	...
	pTableFrame->PerformSitDownAction(wRequestChairID,pIServerUserItem, dwSocketID,pUserSitDown->szPassword);
...
{
	//效验数据
	auto size = sizeof(CMD_GR_CreateTable);
	...
	for(INT_PTR i = 0; i < nSize; ++i)
	{
		...
		if(pTableFrame->GetNullChairCount() == pTableFrame->GetChairCount() && pTableFrame->IsPersonalTableLocked() == false)
		{
			...
			CreateTable.cbTableType = pCreateTable->cbTableType;
...
bool CAttemperEngineSink::OnTCPNetworkSubCancelRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	pCancelRequest->dwWaitTime = DISMISS_WAITE_TIME;
	...
	//容错机制：如果多人同时点击解散房间，房间已经解散后，再点击确定按钮，则直接返回
	if (pCancelRequest->dwTableID == 65535 || pCancelRequest->dwChairID == 65535)
		return false;
	...
	if (pTableFrame->CancelTableRequestReply(pCancelRequest->dwUserID, 1, isDismiss))
	{
		if (isDismiss)
		{
			return true;
...
bool CAttemperEngineSink::OnTCPNetworkSubRequestReply(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//容错机制：如果多人同时点击解散房间，房间已经解散后，再点击确定按钮，则直接返回
	if (pRequestReply->dwTableID == 65535)
		return false;
	...
	bool isDismiss = false;
	if(pTableFrame->CancelTableRequestReply(pRequestReply->dwUserID, pRequestReply->cbAgree, isDismiss) == false) return false;
...
bool CAttemperEngineSink::OnTCPNetworkSubHostDissumeTable(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (pTableFrame->GetTableType() == 0)
	{
		CancelCreateTable.dwUserID[0] = pTableFrame->GetTableOwner();
		CancelCreateTable.cbPlayerCnt = 1;
	}
	else
	{
		DWORD dwUserIDArr[4];
		ZeroMemory(dwUserIDArr, sizeof(dwUserIDArr));
		auto userList = pTableFrame->GetTableUserList();
		int i = 0;
		for (auto &item : userList)
		{
			dwUserIDArr[i++] = item;
		}
		CancelCreateTable.cbPlayerCnt = i;
		CopyMemory(CancelCreateTable.dwUserID, dwUserIDArr, sizeof(CancelCreateTable.dwUserID));
	}
	...
	// 更新房卡信息
	m_pIDBCorrespondManager->PostDataBaseRequest(pCancelRequest->dwUserID, DBR_GR_HOST_CANCEL_CREATE_TABLE, dwSocketID, &CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));

	// 退还费用等逻辑
	if (!pTableFrame->HostDissumeGame(true))
	{
		return true;
	}
...
bool CAttemperEngineSink::InitTableFrameArray()
{
	...
	TableFrameParameter.pIDBCorrespondManager = m_pIDBCorrespondManager;
...
//取消创建
VOID CAttemperEngineSink::CancelCreateTable(DWORD dwUserID[], BYTE cbPlayerCnt, BYTE cbTableType, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID, TCHAR * szRoomID)
{
	//退还费用
	DBR_GR_CancelCreateTable CancelCreateTable;
	ZeroMemory(&CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
	CopyMemory(CancelCreateTable.dwUserID, dwUserID, sizeof(CancelCreateTable.dwUserID));
	CancelCreateTable.cbPlayerCnt = cbPlayerCnt;
	...
	CancelCreateTable.dwServerID = m_pGameServiceOption->wServerID;
	CancelCreateTable.cbTableType = cbTableType;
	lstrcpyn(CancelCreateTable.szRoomID,  szRoomID, sizeof(CancelCreateTable.szRoomID) );

	IServerUserItem* pServerUserItem = m_ServerUserManager.SearchUserItem(dwUserID[0]);
	...
	if(pServerUserItem != NULL)
	{
		...
		//投递数据
		m_pIDBCorrespondManager->PostDataBaseRequest(dwUserID[0], DBR_GR_CANCEL_CREATE_TABLE, dwSocketID, &CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
	}
	else
	{
		//投递数据
		...
		m_pIDBCorrespondManager->PostDataBaseRequest(CancelCreateTable.dwUserID[0], DBR_GR_HOST_CANCEL_CREATE_TABLE, 0, &CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
	}
