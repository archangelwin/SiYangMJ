...
#define DISMISS_WAITE_TIME      20										//超时时间
#define IDI_DISMISS_WAITE_END					(TIME_TABLE_SINK_RANGE+26)
...
public:
	...
	//取消创建
	virtual VOID CancelCreateTable(DWORD dwUserID[], BYTE cbPlayerCnt, BYTE cbTableType, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID, TCHAR * szRoomID);
	//开始游戏写入参与信息
...
protected:
	...
	bool OnDBJoinTableAASucess(DWORD dwContextID, VOID * pData, WORD wDataSize);
	
	bool OnDBJoinTableAAFailure(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//取消创建
...
protected:
	...
	bool OnTCPNetworkSubQuitRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//取消请求
