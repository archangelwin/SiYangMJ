...
bool __cdecl CDBCorrespondManager::OnPostRequestComplete(DWORD dwUserID, bool bSucceed)
{
	...
	if (nIndex != -1)
	{
		...
		for (auto iter = m_DBRequestUserList.begin(); iter != m_DBRequestUserList.end();)
		{
			if (iter->dwUserID == dwUserID)
			{
				iter = m_DBRequestUserList.erase(iter);
				break;
			}
			++iter;
		}
...
VOID CDBCorrespondManager::PerformAmortisation()
{
	...
	while(pos != NULL)
	{
		...
		if(!IsPostDBRequest(pDBRequestHead->dwUserID))
		{
			...
			{
				tagTest item;
				item.dwUserID = pDBRequestHead->dwUserID;
				item.wRequestID = pDBRequestHead->wRequestID;
				m_DBRequestUserList.push_back(item);
