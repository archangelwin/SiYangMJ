...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_CANCEL_CREATE_TABLE:	//取消创建
		{
			DWORD dwCancelUserID[GAME_PLAYER];
			ZeroMemory(dwCancelUserID, sizeof(dwCancelUserID));
			bSucceed = OnRequestCancelCreateTable(dwContextID,pData,wDataSize, dwCancelUserID);
			//协调通知
			for (int i = 0; i < GAME_PLAYER; ++i)
			{
				if (dwCancelUserID[i] == 0)
				{
					break;
				}
				...
				{
					m_pIDBCorrespondManager->OnPostRequestComplete(dwCancelUserID[i], bSucceed);
				}
			}
			return bSucceed;
		}
	...
	case DBR_GR_HOST_CANCEL_CREATE_TABLE:	//取消创建
		{
			DWORD dwCancelUserID[GAME_PLAYER];
			bSucceed = OnRequestHostCancelCreateTable(dwContextID,pData,wDataSize, dwCancelUserID);
			//协调通知
			for (int i = 0; i < GAME_PLAYER; ++i)
			{
				if (dwCancelUserID[i] == 0)
	...
	case DBR_GR_Join_TableAA_COST:			//创建桌子
		{
			bSucceed = OnRequestJoinTableAA(dwContextID, pData, wDataSize, dwUserID);
		}
	break;
	}
...
bool CDataBaseEngineSink::OnRequestCreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		// if (pCreateTable->cbTableType == 0)
		{
			...
			m_TreasureDBAide.AddParameter(TEXT("@dwTableType"), pCreateTable->cbTableType);
			...
			if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_CreateTableFee"), true) == DB_SUCCESS)
			{
				...
				CreateSuccess.cbTableType = pCreateTable->cbTableType;
			...
			else
			{
				...
				if (DB_SUCCESS != CreateFailure.lErrorCode)
				{
...
//取消创建 退还房间
bool CDataBaseEngineSink::OnRequestCancelCreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD dwUserID[GAME_PLAYER])
{
	try
	{
		...
		CopyMemory(CancelCreateResult.dwUserID, pCancelCreateTable->dwUserID, sizeof(CancelCreateResult.dwUserID));
		...
		{
			// m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pCancelCreateTable->dwUserID[0]);
			if (pCancelCreateTable->cbTableType == 0)
			{
				pCancelCreateTable->cbPlayerCnt = 1;
				DWORD userId = pCancelCreateTable->dwUserID[0];
				ZeroMemory(pCancelCreateTable->dwUserID, sizeof(pCancelCreateTable->dwUserID));
				pCancelCreateTable->dwUserID[0] = userId;
			}

			for (int i = 0; i < 4; ++i)
			{
				if (i >= pCancelCreateTable->cbPlayerCnt)
				{
					break;
				}

				dwUserID[i] = pCancelCreateTable->dwUserID[i];

				//构造参数
				m_TreasureDBAide.ResetParameter();
				m_TreasureDBAide.AddParameter(TEXT("@cbTableType"), pCancelCreateTable->cbTableType);

				std::string strUserId = "@dwUserID"; //+ std::to_string(i);
				_bstr_t bstrTmp(strUserId.c_str());
				LPCTSTR pszDescribeString = (LPCTSTR)bstrTmp;
				m_TreasureDBAide.AddParameter(pszDescribeString, pCancelCreateTable->dwUserID[i]);

				//m_TreasureDBAide.AddParameter(TEXT("@strPlayerCnt"), pCancelCreateTable->cbPlayerCnt);
				...
				if (DB_SUCCESS != CancelCreateResult.lErrorCode)
				{
...
//取消创建
bool CDataBaseEngineSink::OnRequestHostCancelCreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD dwUserID[GAME_PLAYER])
{
	try
	{
		//校验数据
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_CreateTableQuit"), true) == DB_SUCCESS)
			{
				...
				CurrenceRoomCardAndBeans.dwUserID = pCancelCreateTable->dwUserID[i];
...
//私人配置
bool CDataBaseEngineSink::OnRequestJoinTableAA(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		//校验数据
		ASSERT(wDataSize == sizeof(DBR_GR_JoinTableAA));
		if (wDataSize != sizeof(DBR_GR_JoinTableAA)) return false;

		DBR_GR_JoinTableAA* joinTableAA = (DBR_GR_JoinTableAA*)pData;
		dwUserID = joinTableAA->dwUserID;
		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE *)&joinTableAA->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), joinTableAA->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@dwServerID"), joinTableAA->dwServerID);
		m_TreasureDBAide.AddParameter(TEXT("@dwDrawCountLimit"), joinTableAA->dwDrawCountLimit);
		m_TreasureDBAide.AddParameter(TEXT("@dwDrawTimeLimit"), joinTableAA->dwDrawTimeLimit);
		m_TreasureDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		...
		//LogPrintf("游戏服务器 执行 GSP_GR_CreateTableFee 存储过程 ");
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_JoinTableFeeAA"), true) == DB_SUCCESS)
		{
			//LogPrintf("游戏服务器 执行 GSP_GR_CreateTableFee 存储过程  成功 ");
			//变量定义
			DBO_GR_Join_TableAA_Success joinTableAASuccess;
			ZeroMemory(&joinTableAASuccess, sizeof(DBO_GR_Join_TableAA_Success));

			joinTableAASuccess.dwUserID = joinTableAA->dwUserID;
			joinTableAASuccess.dwTableID = joinTableAA->dwTableID;
			joinTableAASuccess.dwChairId = joinTableAA->wChairID;
			joinTableAASuccess.dCurBeans = m_TreasureDBAide.GetValue_DOUBLE(TEXT("dCurBeans"));
			joinTableAASuccess.lRoomCard = m_TreasureDBAide.GetValue_LONGLONG(TEXT("RoomCard"));

			TCHAR szInfo[260] = { 0 };
			wsprintf(szInfo, TEXT("ptdt *** 扣费 = %d"), joinTableAASuccess.lRoomCard);
			OutputDebugString(szInfo);

			OutputDebugString(TEXT("ryp0tdt *** GSP_GR_JoinTableFeeAA 数据库创建成功"));

			//LogPrintf("游戏服务器 执行 GSP_GR_CreateTableFee 存储过程  成功  发送数据库执行成功消息 ");
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_Join_TableAA_SUCCESS, dwContextID, &joinTableAASuccess, sizeof(DBO_GR_Join_TableAA_Success));
		}
		else
		{
			//LogPrintf("游戏服务器 执行 GSP_GR_CreateTableFee 存储过程  失败 ");
			...
			//构造结构
			DBO_GR_Join_TableAA_Failure joinTableAAFailure;
			ZeroMemory(&joinTableAAFailure, sizeof(DBO_GR_Join_TableAA_Failure));

			//设置变量
			joinTableAAFailure.lErrorCode = m_TreasureDBAide.GetReturnValue();
			if (DB_SUCCESS != joinTableAAFailure.lErrorCode)
			{
				lstrcpyn(joinTableAAFailure.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(joinTableAAFailure.szDescribeString));
			}
			//发送参数
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_Join_TableAA_FAILURE, dwContextID, &joinTableAAFailure, sizeof(DBO_GR_Join_TableAA_Failure));
		}
	}
	catch (...)
	{
