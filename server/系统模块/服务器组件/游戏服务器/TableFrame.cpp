...
CTableFrame::CTableFrame()
{
	...
	fDismissPastTime = 0.0f;
	fOfflinePastTime = 0.0f;
	fEmptyPastTime = 0.0f;
	//游戏变量
	m_lCellScore=0L;
	m_wDrawCount=0;
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus)
{
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if (pIServerUserItem!=NULL)
			{
				...
				if (pIServerUserItem->GetUserStatus()==US_OFFLINE)
				{
					...
					{
						// PerformStandUpAction(pIServerUserItem);  // TODO
					}
				}
				else
				{
	...
	if((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0)
	{
			...
			(m_dwDrawTimeLimit != 0 && m_dwDrawTimeLimit <= dwTimeNow - m_dwTimeBegin))
		{
			...
			{
				m_pITableFrameSink->PersonalTableEnd();
			}
...
//断线事件
bool CTableFrame::OnEventUserOffLine(IServerUserItem * pIServerUserItem, bool isForceLeave /*= false*/)
{
	//参数效验
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
	...
	//游戏用户
	if (!isForceLeave && cbUserStatus!=US_LOOKON)
	{
		//效验用户
		...
		if ((cbUserStatus==US_PLAYING)&&(m_wOffLineCount[wChairID]<MAX_OFF_LINE)
			|| (m_dwPersonalPlayCount != 0 && m_dwDrawCountLimit >= m_dwPersonalPlayCount)
			|| (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL))//  && (m_PersonalTableParameter.cbIsJoinGame && m_dwTableOwnerID == pUserInfo->dwUserID)))
		{
			//用户设置
...
void CTableFrame::resetAgainValue()
{
	m_dwReplyCount = 0;
	ZeroMemory(m_bRequestReply, sizeof(m_bRequestReply));
	kDismissChairIDList.clear();
	kNotAgreeChairIDList.clear();
	fDismissPastTime = 0.0f;
}
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	case IDI_DISMISS_WAITE_END:	/// 解散等待时间 10s
	{
		...
		// 			{
		// 				if (pTableInfo->cbRoomType == Type_Private_Group_Owner && !pTableInfo->bStart)
		// 				{
		// 					continue; /// 群主开的房间，未开始只能群主手动解散。
		// 				}
		// 

		if (!kDismissChairIDList.empty())
		{
			fDismissPastTime += 5.0f;
			if (fDismissPastTime >= DISMISS_WAITE_TIME)
			{
				if (kNotAgreeChairIDList.size() < 1)
				{
					DismissTable();
					resetAgainValue();
				}
	...
	if (dwTimerID == IDI_PERSONAL_BEFORE_BEGAIN_TIME || dwTimerID == IDI_PERSONAL_AFTER_CREATE_ROOM_TIME)
	{
		...
		DWORD dwUserIDArr[4];
		ZeroMemory(dwUserIDArr, sizeof(dwUserIDArr));
		// 		dwUserIDArr[0] = dwUserID;
		// 		m_pIMainServiceFrame->Can
		...
		for (WORD i = 0; i < m_wChairCount; i++)
		{
			...
			dwUserIDArr[i] = pITableUserItem->GetUserID();
		}
		//退还费用
		m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, m_wUserCount, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID, m_szPersonalTableID);//CANCELTABLE_REASON_ENFOCE
		return true;
	}
...
bool CTableFrame::InitializationFrame(WORD wTableID, tagTableFrameParameter & TableFrameParameter)
{
	...
	m_pIDBCorrespondManager= TableFrameParameter.pIDBCorrespondManager;            //数据协调
...
bool CTableFrame::isNotPersonalDis(BYTE cbUserStatus)
{
	if (!m_bGameStarted)
	{
		return false;
...
bool CTableFrame::MyMethod(IServerUserItem * pIServerUserItem, bool bTreasureDB, DWORD dwTimeNow)
{
...
bool CTableFrame::PerformStandUpAction(IServerUserItem * pIServerUserItem, bool bInitiative/*=false*/)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	ASSERT(pIServerUserItem->GetTableID()==m_wTableID || m_wTableID == 0);
...
// 	return true;
	...
	//游戏用户
	if (isNotPersonalDis(cbUserStatus) || MyMethod(pIServerUserItem, bTreasureDB, dwTimeNow))
	{	
		//比赛类型，私人房间类型
		if((m_pGameServiceOption->wServerType==GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType==MATCH_TYPE_IMMEDIATE)||
	...
	if (pIServerUserItem==pITableUserItem)
	{
		...
		{	
				...
				m_dwPersonalPlayCount != 0 && m_dwTableOwnerID == pIServerUserItem->GetUserID() && m_bPersonalLock == true )
			{
				...
				DWORD dwUserIDArr[4];
				ZeroMemory(dwUserIDArr, sizeof(dwUserIDArr));
				dwUserIDArr[0] = pIServerUserItem->GetUserID();
				
				//退还费用
				if (m_cbTableType == 0)
				{
					dwUserIDArr[0] = m_dwTableOwnerID;
					m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, 1, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID, m_szPersonalTableID);
				}
				else
				{
					BYTE playerCnt = 0;
					for (WORD i = 0; i < m_wChairCount; i++)
					{
						...
						dwUserIDArr[playerCnt++] = pITableUserItem->GetUserID();
					}
					m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, playerCnt, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID, m_szPersonalTableID);
				}
			}
...
//坐下动作
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, DWORD dwSocketID /*= 0*/, LPCTSTR lpszPassword/*=NULL*/)
{
	//// 如果私人房已经结束
	...
	if (m_cbTableType == 1) // AA 加入的时候要扣费
	{
		//构造数据
		DBR_GR_JoinTableAA CreateTableCost;
		ZeroMemory(&CreateTableCost, sizeof(CreateTableCost));

		CreateTableCost.dwUserID = pIServerUserItem->GetUserID();
		// DBR_GR_CreateTableCost.dwClientAddr = pBindParameter->dwClientAddr;  TODO
		CreateTableCost.dwServerID = m_pGameServiceOption->wServerID;
		CreateTableCost.cbTableType = m_cbTableType;
		CreateTableCost.dwTableID = m_wTableID;
		CreateTableCost.wChairID = wChairID;
		CreateTableCost.dwDrawCountLimit = m_dwDrawCountLimit;
		CreateTableCost.dwDrawTimeLimit = m_dwDrawTimeLimit;
		//投递数据  // 如果有错误怎么来不让往下走  TODO
		m_pIDBCorrespondManager->PostDataBaseRequest(CreateTableCost.dwUserID, DBR_GR_Join_TableAA_COST, dwSocketID, &CreateTableCost, sizeof(CreateTableCost));
		return true;
	}

	return PerfomSitDown(pIServerUserItem, wChairID);
}

bool CTableFrame::PerfomSitDown(IServerUserItem * pIServerUserItem, WORD wChairID)
{
	tagUserInfo * pUserInfo = pIServerUserItem->GetUserInfo();
	tagUserRule * pUserRule = pIServerUserItem->GetUserRule();
	IServerUserItem * pITableUserItem = GetTableUserItem(wChairID);

	auto iter = std::find(m_TableUserList.begin(), m_TableUserList.end(), pIServerUserItem->GetUserID());
	if (iter == m_TableUserList.end())
	{
		m_TableUserList.push_back(pIServerUserItem->GetUserID());
	}
...
bool CTableFrame::CancelTableRequest(DWORD dwUserID, WORD dwChairID)
{
	...
	if(m_wUserCount == 1 && pUserItem->GetUserID() == m_dwTableOwnerID)//m_PersonalTableParameter.cbIsJoinGame)
	{
		...
		ZeroMemory(dwUserIDArr, sizeof(dwUserIDArr));
		dwUserIDArr[0] = dwUserID;
		m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, m_wUserCount, m_cbTableType, m_dwDrawCountLimit,  m_dwDrawTimeLimit, CANCELTABLE_REASON_ENFOCE, m_wTableID, m_szPersonalTableID);
		m_dwPersonalPlayCount = 0;
...
//解散答复
bool CTableFrame::CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree, bool& isDissmiss)
{
	//查找用户
	...
	{
		resetAgainValue();
		SetGameTimer(IDI_DISMISS_WAITE_END, 5000L, TIMES_INFINITY, 0);
	}
	...
	if(cbAgree == 0)
	{
		...
		kNotAgreeChairIDList.push_back(wChairID);
	}
	else
	{
		m_bRequestReply[wChairID] = true;			//同意
		kDismissChairIDList.push_back(wChairID);
	}
	m_dwReplyCount++;
	...
	if ((m_dwReplyCount < wOnlineCount))//  && (m_dwReplyCount < (m_wChairCount / 2 + 1)))
	{
		return true;
	}
	...
	if (wAgreeCount >= wOnlineCount || wAgreeCount >= (m_wChairCount / 2 + 1) || wOnlineCount <= 1)
	{
		bCancel = true;
	}
	...
	resetAgainValue();
	KillGameTimer(IDI_DISMISS_WAITE_END);

	return true;
}

void CTableFrame::DismissTable()
{
	KillGameTimer(IDI_DISMISS_WAITE_END);
	...
	if (m_dwPersonalPlayCount == 0)		//游戏未开始
	{
		...
		{
			dwUserIDArr[0] = m_dwTableOwnerID;
			m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, 1, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID, m_szPersonalTableID);
		}
		else
		{
			BYTE playerCnt = 0;
			...
			m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, playerCnt, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID, m_szPersonalTableID);
		}
...
// 			if (pTableUserItem == NULL) continue;
// 
// 			pTableUserItem->SetUserStatus((pTableUserItem->GetUserStatus() == US_OFFLINE) ? US_NULL : US_FREE, INVALID_TABLE, INVALID_CHAIR);
// 		}		
	}
	else	//游戏开始
	{
...
// 1. 在外面创建记录里解散
// 2. 房间内解散    TODO 怎么区分
bool CTableFrame::HostDissumeGame(BOOL bIsEnforce /*= false*/ )
{
	if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL && m_dwPersonalPlayCount > 0)
	{
		return false;
	}
	...
	if (!bIsEnforce)
	{
		...
		if (m_dwPersonalPlayCount == 0)
		{
			dwReason = CANCELTABLE_REASON_ENFOCE;
		}
		else
		{
			dwReason = CANCELTABLE_REASON_OVER_TIME;
		}
		...
		{
			dwUserIDArr[0] = m_dwTableOwnerID;
			m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, 1, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, dwReason, m_wTableID, m_szPersonalTableID);
		}
		else
		{
			BYTE playerCnt = 0;
			...
			m_pIMainServiceFrame->CancelCreateTable(dwUserIDArr, playerCnt, m_cbTableType, m_dwDrawCountLimit, m_dwDrawTimeLimit, dwReason, m_wTableID, m_szPersonalTableID);
		}
	}
...
bool CTableFrame::IsAllOffline()
{
	if (GetSitUserCount() == 0)
	{
		return false;
	}
	return GetSitUserCount() == GetOffLineUserCount();
}
