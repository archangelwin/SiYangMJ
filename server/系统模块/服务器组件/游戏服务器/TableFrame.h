...
protected:
	...
	std::vector<DWORD>				m_TableUserList;				//游戏用户
...
protected:
	...
	std::vector<DWORD>				kDismissChairIDList;
	std::vector<DWORD>				kNotAgreeChairIDList;
...
public:
	...
	std::vector<DWORD> GetTableUserList() const { return m_TableUserList; }
...
public:
	...
	BYTE GetTableType() const { return m_cbTableType; }
	void SetTableType(BYTE val) { m_cbTableType = val; }
...
public:
	...
	bool MyMethod(IServerUserItem * pIServerUserItem, bool bTreasureDB, DWORD dwTimeNow);

	bool isNotPersonalDis(BYTE cbUserStatus);

	//旁观动作
	virtual bool PerformLookonAction(WORD wChairID, IServerUserItem * pIServerUserItem);
	//坐下动作
	virtual bool PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, DWORD dwSocketID = 0, LPCTSTR lpszPassword=NULL);

	bool PerfomSitDown(IServerUserItem * pIServerUserItem, WORD wChairID);
...
public:
	//断线事件
	bool OnEventUserOffLine(IServerUserItem * pIServerUserItem, bool isForceLeave = false);
	//积分事件
	bool OnUserScroeNotify(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbReason);

	void resetAgainValue();
	//系统事件
public:
	//时间事件
...
public:
	...
	//解散答复
	bool CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree, bool& isDissmiss);
	...
	//房主强制解散房间
	bool HostDissumeGame(BOOL bIsEnforce = false);
